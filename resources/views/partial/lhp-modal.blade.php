@switch(Route::current()->getName())
    ...
    @case('lhpmelting.SupplyPreparation')
        <form action="{{ url('/lhp-modal/pre-meltingsupply/save') }}" method="POST" enctype="multipart/form-data" onsubmit="DisabledButtomSubmit()">
            @csrf
                <div class="row">
                    <div class="col-6">
                        <div class="form-floating">
    ...
    @case('lhpmelting.SupplyButton')
        <div class="row justify-content-between">
            <div class="border border-danger p-2 mb-2">Anda sudah melakukan preparation dengan NRP : <span class="fw-bold">3551</span>.  Silahkan pilih <br> <span class="fw-bold"> "Lanjutkan"</span> untuk melanjutkan input supply, atau <br> <span class="fw-bold"> "Preparation"</span> Untuk melakukan pergantian NRP atau preparation ulang.</div>
            <div id="form-tambahan"></div>
            <div class="col-4">
                <a class="btn btn-primary" onclick="Preparation_1('{{$forklift}}','{{ $data->material }}')">Preparation</a>
            </div>
            <div class="col-4">
    ...
    @case('lhpmelting.Supplyresume')
        <div class="table-responsive">
            <table id="Table_ResumeForklift" class="table table-striped-columns table-hover table-bordered nowrap display" style="overflow-x: scroll">
                <thead class="table-dark text-center fw-bold">
                    <tr>
                        <th nowrap class="fs-5">JAM</th>
    ...
    @case('lhpmelting.SupplyInput')
        <form action="{{ url('/lhp/modal-meltingsupply/save') }}" method="POST" enctype="multipart/form-data" onsubmit="DisabledButtomSubmit()">
            @csrf
                <div class="row">
                    <div class="col-6 mb-2">
                        <div class="form-floating">
                            <input type="text" class="form-control border-dark fw-bold" id="forklift"
                                name="forklift" required readonly value="{{ $data->forklift }}">
                    ...
                    <div class="col-6 mb-2">
                        <div class="form-floating">
                            <input type="text" class="form-control border-dark fw-bold" id="material"
                                name="material" required readonly value="{{ $data->material }}">
                            <label for="material" class="">MATERIAL</label>
                        </div>
                    </div>
                    <div class="col-12 mb-2">
                        <div class="form-floating">
                            <input type="number" class="form-control border-dark fw-bold" id="berat"
                                name="berat" required value="0" min="0">
                            ...
                            <div class="col-12">
                                <div class="form-check mt-2" id="furnace" name="furnace" required focus>
                                    @foreach ( App\Models\DB_Furnace::all() as $item)
                                        <input type="radio" class="btn-check" name="furnace" id="furnace_{{$loop->iteration}}" autocomplete="off" value="{{ $item->furnace }}">
                                        <label class="btn btn-outline-success fw-bold" for="furnace_{{$loop->iteration}}">{{ $item->furnace }}</label>
                                    @endforeach
                                </div>
                            </div>
                        </div>
