...
@section('content')
    ...
    <div class="row">
        <div class="col">
            <a class="btn btn-sm btn-secondary mb-2 float-start" href="{{ url('/tv/stokingot') }}" target="_blank"><i
                    class="fa-solid fa-tv"></i> Monitoring Stock</a>
        </div>
    </div>
    <div class="row">
        <div class="col">
            <button class="btn btn-info mb-2 float-start me-2" onclick="ButtonSetupingot('setupstockingot')"><i
                    class="fa-solid fa-cubes-stacked"></i> Setup Stock Ingot</button>
            <button class="btn btn-success mb-2 float-start me-2" onclick="ButtonSetupingot('setupdatatimbangan')"><i
                    class="fa-solid fa-weight-scale"></i></i> Data From Scales</button>
        </div>
    </div>
                ...
                am5.ready(function() {
                    ...
                    $(function() {
                        ...
                        socket.on("stockmaterial-{{ $item->id }}", (datasql{{ $item->id }}) => {
                            ...
                            var max = datasql{{ $item->id }}[0]
                                .max_stock // => ini diisi dengan datasql{{ $item->id }}[0].max_stock
                            var min = datasql{{ $item->id }}[0]
                                .min_stock // => ini diisi dengan datasql{{ $item->id }}[0].max_stock
    ...
    <script>
        function ButtonSetupingot(dataa) {
            ...
            } else if (dataa == 'setupdatatimbangan') {
                link = '/modal/formtimbangan';
                judul = 'FROM TIMBANGAN';
            } else {
                link = '';
                judul = '';
            }
